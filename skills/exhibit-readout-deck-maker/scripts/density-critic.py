#!/usr/bin/env python3
"""Lint an exhibit-readout deck before calling it done.
...
def first_title(ts: list[str]) -> str:
    for t in ts:
        if len(t) >= 8:
            return t
...
def lint_slide(i: int, xml: str, mode: str) -> list[str]:
    ...
    if mode == "casecomp":
        if i > 1 and modules < 3 and "annexure" not in blob.lower() and "appendix" not in blob.lower():
            fails.append(f"slide {i}: fewer than 3 modules")
    else:
...
def main() -> int:
    ...
    ap.add_argument("--mode", choices=["casecomp", "readout"], default="readout")
    args = ap.parse_args()
    path = Path(args.pptx)
    if not path.exists():
